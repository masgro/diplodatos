...
```{r echo=TRUE}
#loc <- "https://astrostatistics.psu.edu/datasets/"
...
```{r echo=TRUE}
a = rnorm(10000,mean=4.0,sd=10.0)
mean(a)
sd(a)
b <- normalize_z(a)
mean(b)
...
```{r echo=TRUE}
data_n <- as.data.frame(lapply(data[2:31], normalize_z))
summary(data_n$V3)
summary(data_n$V8)
```
...
data_train <- data_n[msk, ]
data_test  <- data_n[!msk, ] 

data_train_labels <- data[msk, 1]
data_test_labels  <- data[!msk, 1]
```
